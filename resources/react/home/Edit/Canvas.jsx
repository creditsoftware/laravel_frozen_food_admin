...
export default ({label}) => {
    ...
    return <div><Canvas w={w} h={h} label={label} /></div>;
 };
...
function Canvas({ w, h, label }) {
    ...
    useLayoutEffect(() => { canvasPaint(refCanvas.current, label, format, cfg)}, [label, format, cfg, w, h]);
    return <canvas id="canvas" ref={refCanvas} width={w} height={h} />;
}


async function canvasPaint(can, label, format, cfg) {
...
function drawText(ctx, [x, y, w, h], text, size, lineh, id, suffix) {
    size = size / 10;
    y += size;
